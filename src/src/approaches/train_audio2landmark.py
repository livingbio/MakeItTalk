...
class Audio2landmark_model():
    ...
    def __init__(self, opt_parser, jpg_shape=None):
        ...
        '''
        print('Run on device:', opt_parser.DEVICE)
        ...
        self.std_face_id = torch.tensor(self.std_face_id, requires_grad=False, dtype=torch.float).to(opt_parser.DEVICE)
        ...
        print('G: Running on {}, total num params = {:.2f}M'.format(opt_parser.DEVICE, get_n_params(self.G)/1.0e6))
    ...
    def __train_face_and_pos__(self, fls, aus, embs, face_id, smooth_win=31, close_mouth_ratio=.99):
        ...
        z = torch.tensor(torch.zeros(aus.shape[0], 128), requires_grad=False, dtype=torch.float).to(self.opt_parser.DEVICE)
    ...
    #
        ...
        fl_dis_pred = torch.tensor(fl_dis_pred).to(self.opt_parser.DEVICE) * self.opt_parser.amp_pos
    ...
    def __calib_baseline_pred_fls__(self, baseline_pred_fls, ratio=0.5):
        ...
        baseline_pred_fls = torch.tensor(np_fl_dis_pred, requires_grad=False).to(self.opt_parser.DEVICE)
        baseline_pred_fls[:, 48 * 3::3] *= self.opt_parser.amp_lip_x  # mouth x
        baseline_pred_fls[:, 48 * 3 + 1::3] *= self.opt_parser.amp_lip_y  # mouth y
        return baseline_pred_fls
    ...
    def __train_pass__(self, au_emb=None, centerize_face=False, no_y_rotation=False, vis_fls=False):
        ...
        for i, batch in enumerate(dataloader):
            ...
            for key in keys: # ['45hn7-LXDX8']: #['sxCbrYjBsGA']:#
                ...
                inputs_fl, inputs_au, inputs_emb = inputs_fl.to(self.opt_parser.DEVICE), inputs_au.to(self.opt_parser.DEVICE), inputs_emb.to(self.opt_parser.DEVICE)
