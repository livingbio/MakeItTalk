...
class PositionalEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        seq_len = x.size(1)
        x = x + self.pe[:, :seq_len].clone().detach().to(settings.DEVICE)
        return x
...
class Audio2landmark_pos(nn.Module):
    ...
    def forward(self, au, emb, face_id, fls, z, add_z_spk=False, another_emb=None):
        ...
        if(add_z_spk):
            z_spk = torch.tensor(torch.randn(spk_encode.shape)*0.01, requires_grad=False, dtype=torch.float).to(settings.DEVICE)
...
def nopeak_mask(size):
    ...
    np_mask = np_mask.to(settings.DEVICE)
    return np_mask
