...
class Image_translation_block():
    ...
    def __init__(self, opt_parser, single_test=False):
        print('Run on device {}'.format(opt_parser.DEVICE))
        ...
        if(not single_test):
            ...
            self.criterionVGG.to(opt_parser.DEVICE)
    ...
    def __train_pass__(self, epoch, is_training=True):
        ...
        for i, batch in enumerate(self.dataloader):
            ...
            if(self.opt_parser.comb_fan_awing):
                ...
                audio_in = audio_in.reshape(-1, 1, 256, 256).to(self.opt_parser.DEVICE)
            else:
            ...
            with torch.no_grad():
                ...
                image_in, image_out = \
                    image_in.reshape(-1, 3, 256, 256).to(self.opt_parser.DEVICE), image_out.reshape(-1, 3, 256, 256).to(self.opt_parser.DEVICE)
                inputs = image_out
            ...
            image_fls_in = torch.tensor(img_fls, requires_grad=False).to(self.opt_parser.DEVICE)
            ...
            # image_in, image_out = \
            #     image_in.reshape(-1, 6, 256, 256).to(self.opt_parser.DEVICE), image_out.reshape(-1, 3, 256, 256).to(self.opt_parser.DEVICE)
    ...
    def test(self):
        ...
        for i, batch in enumerate(self.dataloader):
            ...
            image_in, image_out = image_in.to(self.opt_parser.DEVICE), image_out.to(self.opt_parser.DEVICE)
    ...
    def single_test(self, jpg=None, fls=None, filename=None, prefix='', grey_only=False):
        ...
        writer = cv2.VideoWriter(f'{prefix}.mp4', cv2.VideoWriter_fourcc(*'mjpg'), 62.5, (256 * 3, 256))
        ...
        os.system('ffmpeg -loglevel error -y -i {}.mp4 -i {} -pix_fmt yuv420p -strict -2 examples/{}_{}.mp4'.format(
            prefix,
            'examples/'+filename[9:-16]+'.wav',
            prefix, filename[:-4]))
        os.system(f'rm {prefix}.mp4')
